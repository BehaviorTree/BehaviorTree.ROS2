 ...
 *
 * class SubscriberNode: RosTopicSubSharedNode<std_msgs::msg::String>
 *
...
class RosTopicSubSharedNode : public BT::ConditionNode
{
 public:
  // Type definitions
 ...
 protected:
  struct SubscriberInstance
  {
    ...
    // The callback will broadcast to all the instances of RosTopicSubSharedNode<T>
      auto callback = [this](const std::shared_ptr<TopicT> msg)
      {
        // RCLCPP_INFO(rclcpp::get_logger("test"), "new msg");
        broadcaster(msg);
      };
  ...
  static std::unordered_map<std::string, std::shared_ptr<SubscriberInstance>> registry_;
 ...
 public:
   ...
   * */
  explicit RosTopicSubSharedNode(const std::string & instance_name,
                           const BT::NodeConfig& conf,
                           const RosNodeParams& params);

  virtual ~RosTopicSubSharedNode()
  {
    signal_connection_.disconnect();
    // remove the subscribers from the static registry when the ALL the
    // instances of RosTopicSubSharedNode are destroyed (i.e., the tree is destroyed)
    if(sub_instance_)
    {
      sub_instance_.reset();
      std::unique_lock lk(registryMutex());
      auto& registry_ = getRegistry();
      auto it = registry_.find(subscriber_key_);
      // when the reference count is 1, means that the only instance is owned by the
      // registry itself. Delete it
      if(it != registry_.end() && it->second.use_count() <= 1)
      {
        registry_.erase(it);
...
template<class T, class D> inline
  RosTopicSubSharedNode<T, D>::RosTopicSubSharedNode(const std::string & instance_name,
...
template<class T, class D> inline
  bool RosTopicSubSharedNode<T, D>::createSubscriber(const std::string& topic_name)
{
  if(topic_name.empty())
  {
  ...
  if(!shared_resource_initialized && it == registry_.end())
  {
    it = registry_.insert( {subscriber_key_, std::make_shared<SubscriberInstance>() }).first;
  ...
  else if(it == registry_.end())
  {
    throw RuntimeError("Class shared resource has been initialized but is not in the registry!");
  }
  else
 {
    sub_instance_ = it->second;
    RCLCPP_INFO(logger(),
      "Node [%s] retrieved Subscriber to topic [%s]",
      name().c_str(), topic_name.c_str() );
  }
...
template<class T, class D> inline
  NodeStatus RosTopicSubSharedNode<T, D>::tick()
{
  ...
  auto CheckStatus = [](NodeStatus status)
  {
    ...
    {
      throw std::logic_error("RosTopicSubSharedNode: the callback must return"
                             "either SUCCESS or FAILURE");
    }
    return status;
  };

  // RCLCPP_INFO(rclcpp::get_logger("test"), "Tick, before spinsome");
...
template <class TopicT, class Derived>
bool RosTopicSubSharedNode<TopicT, Derived>::shared_resource_initialized = false;
